...
const generate_spec_decomp_mainnet = () => {
  const extract_it_body = (): Map<string, string> => {
    const res = new Map<string, string>();
    try {
      const input = fs.readFileSync('./tests/decomp-mainnet.spec.ts');

      const frx = /describe\('decomp_mainnet', async \(\) => {(.|\n)+/g
  ...
  for (const id of mainnet_contracts) {
    const default_body = `
    const ref = "./michelson/mainnet/${id}.tz"
    const act = "./archetype/mainnet/${id}.arl"
...
${(items.map(x => x)).join('')}
  ...
  fs.writeFileSync('./tests/decomp-mainnet.spec.ts', output)
}

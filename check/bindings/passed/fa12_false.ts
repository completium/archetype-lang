...
const transfer_arg_to_mich = (from_: att.Int): att.Micheline => {
    return from_.to_mich();
}
export class Fa12_false {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/fa12_false.arl", {}, params)).address;
        this.address = address;
    }
    async transfer(from_: att.Int, params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "transfer", transfer_arg_to_mich(from_), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_transfer_param(from_: att.Int, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "transfer", transfer_arg_to_mich(from_), params);
        }
        throw new Error("Contract not initialised");
    }
...
export const fa12_false = new Fa12_false();

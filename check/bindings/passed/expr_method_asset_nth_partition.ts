...
export class Expr_method_asset_nth_partition {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/expr_method_asset_nth_partition.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Option<att.Nat>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Option.from_mich((storage as att.Mpair).args[2], x => { return att.Nat.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const expr_method_asset_nth_partition = new Expr_method_asset_nth_partition();

...
export class Asset_access {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/asset_access.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_y(): Promise<att.Option<att.Bytes>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Option.from_mich((storage as att.Mpair).args[2], x => { return att.Bytes.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
...
export const asset_access = new Asset_access();

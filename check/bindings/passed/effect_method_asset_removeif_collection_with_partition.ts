...
export class my_asset_value implements att.ArchetypeType {
    constructor(public value: att.Int, public c: Array<att.Nat>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.value.to_mich(), att.list_to_mich(this.c, x => {
                return x.to_mich();
            })]);
    }
...
export class Effect_method_asset_remove_if_collection_with_partition {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/effect_method_asset_remove_if_collection_with_partition.arl", {}, params)).address;
        this.address = address;
    }
...
export const effect_method_asset_remove_if_collection_with_partition = new Effect_method_asset_remove_if_collection_with_partition();

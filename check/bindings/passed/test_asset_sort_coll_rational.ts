...
export class Test_asset_sort_coll_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/test_asset_sort_coll_rational.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map((storage as att.Mpair).args[0], (x, y) => [att.Nat.from_mich(x), att.Rational.from_mich(y)]);
        }
        throw new Error("Contract not initialised");
    }
...
export const test_asset_sort_coll_rational = new Test_asset_sort_coll_rational();

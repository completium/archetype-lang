...
export class Import_arl_enum_use_all {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/import_arl_enum_use_all.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<import_arl_all_def__my_enum> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return mich_to_import_arl_all_def__my_enum(storage);
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const import_arl_enum_use_all = new Import_arl_enum_use_all();

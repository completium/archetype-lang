...
export class my_record implements att.ArchetypeType {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.t.to_mich((x => { return x.to_mich((x => { return att.string_to_mich(x); })); })), this.n.to_mich(), att.string_to_mich(this.s), att.pair_to_mich([att.string_to_mich(this.v[0]), this.v[1].to_mich(), this.v[2].to_mich((x => { return x.to_mich(); }))])]);
    }
...
export class Ticket_record_list_var_3_1 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/ticket_record_list_var_3_1.arl", {}, params)).address;
        this.address = address;
    }
...
export const ticket_record_list_var_3_1 = new Ticket_record_list_var_3_1();

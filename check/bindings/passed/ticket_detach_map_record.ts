...
export class Ticket_detach_map_record {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/ticket_detach_map_record.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_input(): Promise<Array<[
        ...
        if (this.address != undefined) {
            ...
            return att.mich_to_map((storage as att.Mpair).args[0], (x, y) => [att.Nat.from_mich(x), my_record.from_mich(y)]);
        }
        throw new Error("Contract not initialised");
    }
...
export const ticket_detach_map_record = new Ticket_detach_map_record();

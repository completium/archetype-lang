...
export class Match_detach_map {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/match_detach_map.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_mt(): Promise<Array<[
        att.Address,
        att.Ticket<att.Unit>
    ]>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.mich_to_map(storage, (x, y) => [att.Address.from_mich(x), att.Ticket.from_mich(y, x => { return new att.Unit(); })]);
        }
        throw new Error("Contract not initialised");
    }
    errors = {
...
export const match_detach_map = new Match_detach_map();

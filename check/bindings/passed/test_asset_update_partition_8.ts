...
export class o_asset_value implements att.ArchetypeType {
    constructor(public v: string, public i: att.Int) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): o_asset_value {
        return new o_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.Int.from_mich((input as att.Mpair).args[1]));
    }
}
...
export class Test_asset_update_partition_8 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/test_asset_update_partition_8.arl", {}, params)).address;
        this.address = address;
    }
...
export const test_asset_update_partition_8 = new Test_asset_update_partition_8();

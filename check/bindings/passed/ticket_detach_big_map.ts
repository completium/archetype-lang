...
export class Ticket_detach_big_map {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/ticket_detach_big_map.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_input_value(key: att.Nat): Promise<att.Ticket<string> | undefined> {
        if (this.address != undefined) {
            ...
            if (data != undefined) {
                return att.Ticket.from_mich(data, x => { return att.mich_to_string(x); });
            }
            else {
                return undefined;
            }
...
export const ticket_detach_big_map = new Ticket_detach_big_map();

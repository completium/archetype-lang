...
export class Decomp_map {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/decomp_map.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_s(): Promise<att.Option<string>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
...
export const decomp_map = new Decomp_map();

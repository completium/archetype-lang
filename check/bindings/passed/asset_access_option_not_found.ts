...
export class Asset_access_option_not_found {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/asset_access_option_not_found.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_abc(): Promise<abc_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map((storage as att.Mpair).args[0], (x, y) => [att.Nat.from_mich(x), abc_value.from_mich(y)]);
        }
        throw new Error("Contract not initialised");
    }
...
export const asset_access_option_not_found = new Asset_access_option_not_found();

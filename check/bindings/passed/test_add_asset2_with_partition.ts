...
export class my_asset_value implements att.ArchetypeType {
    constructor(public p: Array<att.Nat>, public v: boolean) { }
    toString(): string {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.list_to_mich(this.p, x => {
                return x.to_mich();
            }), att.bool_to_mich(this.v)]);
    }
    equals(v: my_asset_value): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.mich_to_list((input as att.Mpair).args[0], x => { return att.Nat.from_mich(x); }), att.mich_to_bool((input as att.Mpair).args[1]));
    }
}
...
export class Test_add_asset2_with_partition {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/test_add_asset2_with_partition.arl", {}, params)).address;
        this.address = address;
    }
...
export const test_add_asset2_with_partition = new Test_add_asset2_with_partition();

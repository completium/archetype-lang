...
export class my_asset_value implements att.ArchetypeType {
    constructor(public v: att.Nat, public i: att.Int) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.v.to_mich(), this.i.to_mich()]);
    }
    equals(v: my_asset_value): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.Nat.from_mich((input as att.Mpair).args[0]), att.Int.from_mich((input as att.Mpair).args[1]));
    }
}
export const my_asset_value_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("nat", ["%v"]),
    att.prim_annot_to_mich_type("int", ["%i"])
], []);
...
export class Effect_method_asset_update_all_coll_2 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/effect_method_asset_update_all_coll_2.arl", {}, params)).address;
        this.address = address;
    }
...
export const effect_method_asset_update_all_coll_2 = new Effect_method_asset_update_all_coll_2();

...
export class o_asset_value implements att.ArchetypeType {
    constructor(public v: boolean, public a: att.Nat) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.bool_to_mich(this.v), this.a.to_mich()]);
    }
    equals(v: o_asset_value): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): o_asset_value {
        return new o_asset_value(att.mich_to_bool((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export class Test_asset_nth_agg_2 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/test_asset_nth_agg_2.arl", {}, params)).address;
        this.address = address;
    }
...
export const test_asset_nth_agg_2 = new Test_asset_nth_agg_2();

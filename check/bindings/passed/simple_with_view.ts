...
const assign_arg_to_mich = (): att.Micheline => {
...
export class Simple_with_view {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/simple_with_view.arl", {}, params)).address;
        this.address = address;
        this.getValue_callback_address = (await deploy_getValue_callback(params)).address;
    }
    async assign(params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "assign", assign_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_assign_param(params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "assign", assign_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }
...
export const simple_with_view = new Simple_with_view();

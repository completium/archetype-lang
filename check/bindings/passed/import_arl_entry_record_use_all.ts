...
export class Import_arl_entry_record_use_all {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/import_arl_entry_record_use_all.arl", {}, params)).address;
        this.address = address;
    }
    async set_res(r: import_arl_all_def__my_record, params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "set_res", set_res_arg_to_mich(r), params);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_set_res_param(r: import_arl_all_def__my_record, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "set_res", set_res_arg_to_mich(r), params);
        }
        throw new Error("Contract not initialised");
    }
...
export const import_arl_entry_record_use_all = new Import_arl_entry_record_use_all();

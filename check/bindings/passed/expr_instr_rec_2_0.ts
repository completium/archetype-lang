...
export class my_record implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.mich_to_map((input as att.Mpair).args[0], (x, y) => [att.Nat.from_mich(x), att.mich_to_string(y)]), att.Nat.from_mich((input as att.Mpair).args[1]));
    }
}
...
export class Expr_instr_rec_2_0 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/expr_instr_rec_2_0.arl", {}, params)).address;
        this.address = address;
    }
...
export const expr_instr_rec_2_0 = new Expr_instr_rec_2_0();

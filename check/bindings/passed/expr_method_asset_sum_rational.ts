...
export class Expr_method_asset_sum_rational {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/expr_method_asset_sum_rational.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset(): Promise<my_asset_container> {
        if (this.address != undefined) {
            ...
            return att.mich_to_map((storage as att.Mpair).args[0], (x, y) => [att.mich_to_string(x), att.Rational.from_mich(y)]);
        }
        throw new Error("Contract not initialised");
    }
...
export const expr_method_asset_sum_rational = new Expr_method_asset_sum_rational();

...
export class Iterable_big_map_get {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/iterable_big_map_get.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<att.Option<att.Bytes>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Option.from_mich((storage as att.Mpair).args[1], x => { return att.Bytes.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {
...
export const iterable_big_map_get = new Iterable_big_map_get();

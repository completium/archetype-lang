...
export class my_event implements att.ArchetypeType {
    ...
    static from_mich(input: att.Micheline): my_event {
        return new my_event(att.mich_to_string((input as att.Mpair).args[0]), att.Nat.from_mich((input as att.Mpair).args[1]), att.Bytes.from_mich((input as att.Mpair).args[2]));
    }
}
const exec_arg_to_mich = (): att.Micheline => {
    return att.unit_mich;
}
export class Import_arl_event_use_collide {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/import_arl_event_use_collide.arl", {}, params)).address;
        this.address = address;
    }
...
export const import_arl_event_use_collide = new Import_arl_event_use_collide();

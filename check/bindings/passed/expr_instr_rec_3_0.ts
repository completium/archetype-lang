...
export class my_record implements att.ArchetypeType {
    constructor(public m: Array<[
        att.Nat,
        string
    ]>, public id: att.Nat, public a: string) { }
    toString(): string {
    ...
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.list_to_mich(this.m, x => {
            ...
            }), this.id.to_mich(), att.string_to_mich(this.a)]);
    }
    equals(v: my_record): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): my_record {
        return new my_record(att.mich_to_map((input as att.Mpair).args[0], (x, y) => [att.Nat.from_mich(x), att.mich_to_string(y)]), att.Nat.from_mich((input as att.Mpair).args[1]), att.mich_to_string((input as att.Mpair).args[2]));
    }
}
...
export class Expr_instr_rec_3_0 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/expr_instr_rec_3_0.arl", {}, params)).address;
        this.address = address;
    }
...
export const expr_instr_rec_3_0 = new Expr_instr_rec_3_0();

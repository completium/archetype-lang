...
const a_arg_to_mich = (): att.Micheline => {
...
export class Test_getter2 {
    ...
    async a(params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "a", a_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_a_param(params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "a", a_arg_to_mich(), params);
        }
        throw new Error("Contract not initialised");
    }

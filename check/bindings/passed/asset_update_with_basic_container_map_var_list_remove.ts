...
export class my_asset_value implements att.ArchetypeType {
    constructor(public v0: att.Nat, public c: Array<[
        att.Nat,
        string
    ]>) { }
    toString(): string {
    ...
    static from_mich(input: att.Micheline): my_asset_value {
        return new my_asset_value(att.Nat.from_mich((input as att.Mpair).args[0]), att.mich_to_map((input as att.Mpair).args[1], (x, y) => [att.Nat.from_mich(x), att.mich_to_string(y)]));
    }
}
export const my_asset_value_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    att.prim_annot_to_mich_type("nat", ["%v0"]),
    att.pair_annot_to_mich_type("map", att.prim_annot_to_mich_type("nat", []), att.prim_annot_to_mich_type("string", []), ["%c"])
], []);
...
export class Asset_update_with_basic_container_map_var_list_remove {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/asset_update_with_basic_container_map_var_list_remove.arl", {}, params)).address;
        this.address = address;
    }
...
export const asset_update_with_basic_container_map_var_list_remove = new Asset_update_with_basic_container_map_var_list_remove();

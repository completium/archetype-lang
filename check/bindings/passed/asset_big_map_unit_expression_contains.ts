...
export class Asset_big_map_unit_expression_contains {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/asset_big_map_unit_expression_contains.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_my_asset_value(key: att.Nat): Promise<my_asset_value | undefined> {
        if (this.address != undefined) {
            ...
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair).args[0]).toString()), key.to_mich(), my_asset_key_mich_type);
            if (data != undefined) {
...
export const asset_big_map_unit_expression_contains = new Asset_big_map_unit_expression_contains();

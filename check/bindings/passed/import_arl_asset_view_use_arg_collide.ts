...
const exec_imported_arg_to_mich = (i: Array<att.Nat>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return x.to_mich();
    });
}
const exec_top_arg_to_mich = (i: Array<att.Bytes>): att.Micheline => {
    return att.list_to_mich(i, x => {
        return x.to_mich();
    });
}
export class Import_arl_asset_view_use_arg_collide {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/import_arl_asset_view_use_arg_collide.arl", {}, params)).address;
        this.address = address;
    }
    async exec_imported(i: Array<att.Nat>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "exec_imported", exec_imported_arg_to_mich(i), params);
        }
        throw new Error("Contract not initialised");
    }
    async exec_top(i: Array<att.Bytes>, params: Partial<ex.Parameters>): Promise<att.CallResult> {
        if (this.address != undefined) {
            return await ex.call(this.address, "exec_top", exec_top_arg_to_mich(i), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_exec_imported_param(i: Array<att.Nat>, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "exec_imported", exec_imported_arg_to_mich(i), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_exec_top_param(i: Array<att.Bytes>, params: Partial<ex.Parameters>): Promise<att.CallParameter> {
        if (this.address != undefined) {
    ...
    async get_res_imported(): Promise<att.Option<Array<att.Nat>>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Option.from_mich((storage as att.Mpair).args[1], x => { return att.mich_to_list(x, x => { return att.Nat.from_mich(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    async get_res_top(): Promise<att.Option<Array<att.Bytes>>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.Option.from_mich((storage as att.Mpair).args[2], x => { return att.mich_to_list(x, x => { return att.Bytes.from_mich(x); }); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const import_arl_asset_view_use_arg_collide = new Import_arl_asset_view_use_arg_collide();

...
export class my_asset_value implements att.ArchetypeType {
    constructor(public value: att.Int, public my_partition: Array<att.Nat>) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.value.to_mich(), att.list_to_mich(this.my_partition, x => {
                return x.to_mich();
            })]);
    }
...
export const my_asset_value_mich_type: att.MichelineType = att.pair_array_to_mich_type([
    ...
    att.set_annot_to_mich_type(att.prim_annot_to_mich_type("nat", []), ["%my_partition"])
], []);
...
export class Effect_method_asset_addupdate_with_replace_partition {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/effect_method_asset_addupdate_with_replace_partition.arl", {}, params)).address;
        this.address = address;
    }
...
export const effect_method_asset_addupdate_with_replace_partition = new Effect_method_asset_addupdate_with_replace_partition();

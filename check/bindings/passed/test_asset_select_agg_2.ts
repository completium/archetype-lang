...
export class o_asset_value implements att.ArchetypeType {
    constructor(public s: string, public b: boolean) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([att.string_to_mich(this.s), att.bool_to_mich(this.b)]);
    }
    equals(v: o_asset_value): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): o_asset_value {
        return new o_asset_value(att.mich_to_string((input as att.Mpair).args[0]), att.mich_to_bool((input as att.Mpair).args[1]));
    }
}
...
export class Test_asset_select_agg_2 {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("../tests/passed/test_asset_select_agg_2.arl", {}, params)).address;
        this.address = address;
    }
    ...
    async get_res(): Promise<Array<att.Nat>> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            return att.mich_to_list((storage as att.Mpair).args[2], x => { return att.Nat.from_mich(x); });
        }
        throw new Error("Contract not initialised");
    }
    errors = {};
}
export const test_asset_select_agg_2 = new Test_asset_select_agg_2();

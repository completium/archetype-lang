...
process() {
  ${BIN} -d -dir $1 >/dev/null 2>/dev/null
  if [ $? -eq 0 ]; then
    ${BIN} -d $1 >/tmp/$2.arl 2>/dev/null
    if [ $? -eq 0 ]; then
      ${BIN} /tmp/$2.arl >/dev/null 2>/dev/null
      if [ $? -eq 0 ]; then
        echo "$2,ok,ok,ok" >>$CSV
        echo -e "\033[32m OK \033[0m"
      else
        echo "$2,ok,ok,ko" >>$CSV
        echo -e "\033[31m KO_2 \033[0m"
      fi
    else
      echo "$2,ok,ko,ko" >>$CSV
      echo -e "\033[31m KO_1 \033[0m"
    fi
  else
    echo "$2,ko,ko,ko" >>$CSV
    echo -e "\033[31m KO_0 \033[0m"
  fi
}

echo "Generate mainnet contract csv"

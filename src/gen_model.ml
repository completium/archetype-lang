...
let rec to_model ((_tenv, ast) : Typing.env * A.ast) : M.model =
  ...
  let process_asset (env : env) (a : A.asset) : M.decl_node =
    ...
    let r : M.asset = M.mk_asset (to_mident a.name) ~keys:(List.map unloc (a.keys)) ~values:values ~sort:(List.map M.mk_mident a.sort) ~map_kind:mp ~init:(match a.init with | IAliteral l -> List.map (fun x -> (mk_asset (to_mident a.name)) x) l | IAident _ -> []) ~no_storage ~loc:a.loc in
    M.Dasset r
  in

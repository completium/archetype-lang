...
let rec map_mterm m ctx (mt : M.mterm) : loc_term =
  ...
  let t =
    ...
    | Massign (assignop, t, Arecord (lv, _rn, fn), v) ->
      let k : M.mterm =
        let aux = function
          | M.Avar x
          | M.Avarstore x -> M.mk_mvar x M.tunit
          | _ -> assert false
        in
        aux lv
      in
      let left = map_mterm m ctx k in
      let right : loc_term = with_dummy_loc (Trecord (Some left, [map_lident fn, map_mterm m ctx v])) in
      Tassign (left, get_assign_value t left right assignop)

    | Massign (_, _, Atuple _, _) -> error_not_translated "Massign (_, _, Atuple _, _)"
    ...
    | Mternaryasset _ -> error_not_translated "Mternaryasset"
    ...
    | Misnat s -> Tapp (loc_term (Tvar "isnat"),[map_mterm m ctx s])

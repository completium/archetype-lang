...
import { registerEvent, WellEvent, WellEventProcessor, hex_to_data } from '@completium/well-event-crank';
...
let pp_event fmt (ie : input_event) =
  ...
  in
  let pp_register fmt =
    Format.fprintf fmt "export function register_%a(source : string, handler : WellEventProcessor<%a>) {

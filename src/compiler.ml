...
let main () : unit =
  ...
  let f = function
    ...
    | "debug-trace"       -> Options.opt_g := true; Options.target := DebugTrace
    |  s ->
      Format.eprintf
  ...
  let arg_list = Arg.align [
      ...
      "--sandbox-exec-address", Arg.String (fun s -> Options.opt_sandbox_exec_address := Some s), " Set address contract for sandbox execution";

...
let storage_var : mterm = mk_mterm (Mvar (storage_lident, Vlocal, Tnone, Dnone)) Tstorage

let operations_lident : lident = dumloc "_ops"
let operations_type : type_ = Tcontainer (Toperation, Collection)
let operations_var : mterm = mk_mterm (Mvar (operations_lident, Vlocal, Tnone, Dnone)) operations_type
...
let rec simplify (mt : mterm) : mterm =
  match mt.node with
  | Mletin ([let_id], init, _, {node = (Mvar (var_id, Vlocal, Tnone, Dnone)); _}, _)
  | Mletin ([let_id], init, _, {node = (Mtuple [{node = Mvar (var_id, Vlocal, Tnone, Dnone); _}]); _}, _) when String.equal (unloc let_id) (unloc var_id) -> simplify init
  | Mletin (ids, init, _, {node = (Mtuple tuples); _}, _) when
      (
        ...
        let tuples_ids, continue = List.fold_right (fun (x : mterm) (tuples_ids, continue) ->
            match x.node with
            | Mvar (id, Vlocal, Tnone, Dnone) -> id::tuples_ids, continue
...
let rec process_non_empty_list_term (model : model) (ctx : ctx_red) (s : s_red) (mts : mterm list) : mterm * s_red =
  ...
  List.fold_right (fun x (accu, s) ->
      ...
      | { node = Massign (op, _, Avar id, value); _} ->
        ...
        let value =
          let var = mk_mterm (Mvar (id, Vlocal, Tnone, Dnone)) type_ in
          match op with
          | ValueAssign -> value
...
and process_mtern (model : model) (ctx : ctx_red) (s : s_red) (mt : mterm) : mterm * s_red =
    ...
    let target, subs =
      (match ctx.target with
       | Some {node = (Mtuple _l); _} ->
         ...
         mk_mterm (Mtuple (storage_var::(List.map (fun (x, y : ident * type_) -> mk_mterm (Mvar (dumloc x, Vlocal, Tnone, Dnone)) y) subs)))
    ...
    begin
      ...
      | _ ->
        ...
        let l = (
          ...
          | [] -> l
          | _ ->
            let target = mk_mterm (Mtuple (storage_var::(List.map (fun (x, y : ident * type_) -> mk_mterm (Mvar (dumloc x, Vlocal, Tnone, Dnone)) y) subs)))
  ...
  | Mfor (FIsimple a, ICKview col, body, _) ->
    let col, s = process_mtern model ctx s col in
    ...
    let tuple : mterm = mk_mterm (Mtuple ([storage_var] @ List.map (fun (x, y) -> mk_mterm (Mvar (dumloc x, Vlocal, Tnone, Dnone)) y) subs)) type_tuple in
    let ctx = {
      ctx with
      target = Some tuple;
    } in

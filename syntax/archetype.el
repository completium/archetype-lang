...
;;   `
    (,
      (archetype-regexp-opt '
        ("state" "now" "transferred" "caller" "fail" "balance" "conditions" "effects" "none" "any" "transfer")) . font-lock-constant-face)
;;   `
    (,
      (archetype-regexp-opt '
        ("use" "model" "extension" "constant" "variable" "role" "asset" "states" "enum" "action" "function" "namespace" "contract")) . font-lock-type-face)
;;   `
    (,
      (archetype-regexp-opt '
        ("identified" "sorted" "by" "as" "from" "to" "match" "with" "end" "ref" "fun" "=>" "initialized" "collection" "queue" "stack" "set" "partition" "asset" "initial" "args" "called" "transition" "condition" "effect" "specification" "invariant" "ensure" "let" "if" "then" "else" "for" "in" "break" "of" "back")) . font-lock-keyword-face)
;;   `
    (,
      (archetype-regexp-opt '
        ("and" "or" "not" "->" "<->" "forall" "exists" "assert" "true" "false")) . font-lock-doc-face)
